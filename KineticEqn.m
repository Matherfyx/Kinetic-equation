function [rho_L2_error,j_L2_error,X,N,rhoNumerical,rho_exact] = KineticEqn(xa,xb,t_end,N,qu,qv,eps)
...
while(t<t_end)
    ...
    for i = 1:N
        % D(i,:) = 1./(eps^2+dt)*(eps^2*D0(i,:)'-dt*(-invA3*B3*C(i,:)'+invA3*basisMatrix*[phiflux(i+1);phiflux(i)]))';
        % B(i,:) = 1./(eps^2+dt)*(eps^2*B0(i,:)'-dt*(invA3*B3*C(i,:)'-invA3*basisMatrix*[phiflux(i+1);phiflux(i)]))';
